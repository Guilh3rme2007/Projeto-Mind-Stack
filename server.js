...
app.post("/login", (req, res) => {
  ...
  connection.query(sql, [email], (err, results) => {
    ...
    res
      .status(200)
      .json({ user_id: user.user_id, message: "Login realizado com sucesso" });
  });
});
...
app.post("/notes/create", (req, res) => {
  ...
  const sql =
    "INSERT INTO note (user_id, group_id, content, color, position_x, position_y) VALUES (?, ?, ?, ?, ?, ?)";

  connection.query(
    sql,
    [user_id, group_id, content, color, position_x, position_y],
    (err, result) => {
      if (err) {
      ...
      res
        .status(201)
        .json({ note_id: result.insertId, message: "Nota criada com sucesso" });
    }
  );
});
...
app.post("/notes/update", (req, res) => {
  ...
  const sql = `UPDATE note SET ${fields.join(
    ","
...
app.get("/notes/get/:group_id/:user_id", (req, res) => {
  ...
  const sql =
    "SELECT * FROM note WHERE group_id = ? AND user_id = ? ORDER BY note_id DESC";
...
app.post("/groups/create", (req, res) => {
  ...
  connection.query(sql, [user_id, group_name], (err, result) => {
    ...
    return res
      .status(201)
      .json({ group_id: result.insertId, message: "Grupo criado com sucesso" });
  });
});
...
app.get("/groups/get/:user_id", (req, res) => {
  ...
  const sql =
    "SELECT group_id, group_name FROM note_group WHERE user_id = ? ORDER BY created_at DESC";

...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("click", (event) => {
    if (
      event.target === groupManagerModal &&
      groupManagerModal.classList.contains("active")
    ) {
      closeGroupModal();
    }
  });
...
async function updateNoteColor(id, newColor) {
  try {
    ...
    if (response.ok) {
      console.log(
        `Cor da nota ID ${id} atualizada com sucesso para: ${newColor}`
      );
    } else {
      const errorText = await response.text();
...
async function loadNotesCurrentGroup() {
  ...
  try {
    const response = await fetch(
      `/notes/get/${currentGroupId}/${LOGGED_IN_USER_ID}`
    );
    ...
    if (response.ok) {
      ...
      notes.forEach((note) => {
        const noteElement = creatNoteElement(
          note.note_id,
          note.content,
          note.color
        );
